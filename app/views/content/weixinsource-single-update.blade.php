                                ...
                                <div class="row-fluid">
                                    <div class="span8">
                                        ...
                                        <div class="control-group">
                                            ...
                                            <div class="controls">
                                                <input type="text" id="url" name="url" value="{{{$source->url}}}"
                                                       class="span10 m-wrap popovers" data-trigger="hover"/>
                                            </div>
                                        </div>
                                        <div class="control-group">
                                            ...
                                            <div class="controls">
                                                <textarea class="large m-wrap" rows="3" name="brief" id="brief">{{{$source->brief}}}</textarea>
                                            </div>
                                        </div>
                                        <div class="control-group">
                                    ...
                                    <div class="span4">
                                        <div class="control-group" style="height:260px;width:250px;color:#c0c0c0;margin:0 20px 0 20px;border:1px solid #c0c0c0;">
                                            <div style="margin:5px 0 5px 11px" id="minTitle">{{{$source->title or '标题'}}}</div>
                                            <div style="text-align:center;background-color: #f5f2f2; height:200px;margin:5px 15px 5px 15px;">
                                                @if($source->cover != '')
                                                <img id="jsPicImg" src="{{{$baseURL}}}/{{{$source->cover}}}" data="{{{$source->cover}}}"/>
                                                <div style="padding-top:90px;display: none">封面图片<BR/>(大图片建议尺寸：900像素 * 500像素)</div>
                                                @else
                                                    <img id="jsPicImg" style="display: none;"/>
                                                    <div style="padding-top:90px;">封面图片<BR/>(大图片建议尺寸：900像素 * 500像素)</div>
                                                @endif
                                            </div>
                                            <div style="margin-left:11px" id="minBrief">{{{$source->brief or '简介'}}}</div>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        jQuery(document).ready(function() {
            ...
            $("#JsSaveBtn").on('click', function () {
                ...
                var id = <?php echo $id ?>;
                var data = {"id": id, "title" : title, "type" : type, "cover" : cover, "brief" : brief, "url" : url, "editor" : editor};
